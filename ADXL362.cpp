...
float ADXL362::readGXData()
{
	return int16_tToGFloat(readXData());
}

float ADXL362::readGYData()
{
	return int16_tToGFloat(readYData());
}

float ADXL362::readGZData()
{
	return int16_tToGFloat(readZData());
}
...
float ADXL362::readModule()
{
	int16_t X, Y, Z, temp, module;
	float floatModule;
	readXYZTData(X, Y, Z, temp);
	module = sqrt(pow(X, 2) + pow(Y, 2) + pow(Z, 2));
	floatModule = (float)module / 1024.0;
	return floatModule;
}

float ADXL362::int16_tToGFloat(int16_t int16_tValue)
{
	return (float)int16_tValue / 1024.0;
}

String ADXL362::getXYZModuleAsString(char separator,uint8_t Nbdecimal)
{
	String toPrint;
	toPrint=String(readGXData(),Nbdecimal)+separator;
	toPrint+=String(readGYData(),Nbdecimal)+separator;
	toPrint+=String(readGZData(),Nbdecimal)+separator;
	toPrint+=String(readModule(),Nbdecimal);
	return toPrint;
}
...
bool ADXL362::test()
{
	...
	{
		floatModule=readModule();
		Serial.println(getXYZModuleAsString(';',4));
		min = min(min, floatModule);
		max = max(max, floatModule);
	}
	Serial.println("min" + String(min, 4) + "max" + String(max, 4));
	if (min > 0.8 && max < 1.2)
	{
		Serial.println("accelero OK");
		return true;
	}

	else
	{
		Serial.println("accelero NOT OK");
		return false;
	}
}
...
#ifdef ADXL362_DEBUG
	Serial.print("DC Activity Threshold set to ");
	Serial.print(SPIreadTwoRegisters(0x20));
	Serial.print(", Time threshold set to ");
	Serial.print(SPIreadOneRegister(0x22));
	Serial.print(", ACT_INACT_CTL Register is ");
	Serial.println(ACT_INACT_CTL_Reg, HEX);
#endif
}
...
#ifdef ADXL362_DEBUG
	Serial.print("AC Activity Threshold set to ");
	Serial.print(SPIreadTwoRegisters(0x20));
	Serial.print(", Time Activity set to ");
	Serial.print(SPIreadOneRegister(0x22));
	Serial.print(", ACT_INACT_CTL Register is ");
	Serial.println(ACT_INACT_CTL_Reg, HEX);
#endif
...
#ifdef ADXL362_DEBUG
	Serial.print("DC Inactivity Threshold set to ");
	Serial.print(SPIreadTwoRegisters(0x23));
	Serial.print(", Time Inactivity set to ");
	Serial.print(SPIreadTwoRegisters(0x25));
	Serial.print(", ACT_INACT_CTL Register is ");
	Serial.println(ACT_INACT_CTL_Reg, HEX);
#endif
}
...
#ifdef ADXL362_DEBUG
	Serial.print("AC Inactivity Threshold set to ");
	Serial.print(SPIreadTwoRegisters(0x23));
	Serial.print(", Time Inactivity set to ");
	Serial.print(SPIreadTwoRegisters(0x25));
	Serial.print(", ACT_INACT_CTL Register is ");
